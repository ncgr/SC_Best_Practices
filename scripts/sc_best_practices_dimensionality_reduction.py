...
def run_dimensionality_reduction(h5ad_file, sample_id, normalization="log1p_norm"):
    ...
    ## plot total counts, percentage mt counts, doublet detection score, is doublet in 4 UMAPs
    sc.pl.umap(
        adata,
        color=["total_counts", "pct_counts_mt", "scDblFinder_score", "scDblFinder_class"],
        save=f".{sample_id}.total_mt_dblscore_dbl.umap.png",
    )
    adata.write(f"{sample_id}.dimensionality_reduction.h5ad")
